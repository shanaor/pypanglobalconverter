...
try:
    ...
    print("pyreadstat is not installed. SPSS writing will not be available.")
    print("To install, run: pip install pyreadstat")
...
def read_file(file_path):
    ...
    elif file_type in ['sql', 'db', 'sqlite', 'sqlite3']:
        conn = sqlite3.connect(file_path)
        tables = pd.read_sql_query("SELECT name FROM sqlite_master WHERE type='table';", conn)
        if len(tables) == 0:
            print("No tables found in the database.")
            return None
        table_name = input(f"Available tables: {', '.join(tables['name'])}. Enter table name: ")
...
def write_file(df, output_path, output_type):
    try:
        if output_type == 'csv':
            ...
            print("Writing to SAS format is not supported. You need SAS software or third-party tools.")
        elif output_type == 'spss':
            if PYREADSTAT_AVAILABLE:
                pyreadstat.write_sav(df, output_path)
                print(f"SPSS file has been successfully written to {output_path}")
            else:
                print("Writing to SPSS format requires pyreadstat. Please install it and try again.")
        elif output_type in ['sql', 'db', 'sqlite', 'sqlite3']:
            conn = sqlite3.connect(output_path)
            table_name = input("Enter the table name to write to: ")
            df.to_sql(table_name, conn, if_exists='replace', index=False)
            conn.close()
            print(f"Data written to SQLite database '{output_path}', table '{table_name}'")
...
def main():
    ...
    output_types = [
        ...
        'sas', 'spss', 'sql', 'stata', 'sqlite', 'sqlite3'
    ]
    ...
    try:
        ...
        if df is not None:
            ...
            if output_type != 'clipboard' and output_type not in ['sql', 'db', 'sqlite', 'sqlite3', 'spss']:
